...
def extended():
    ...
    size = "465x459"
    root.geometry(size)
    equalsbtn.config(width="304",image=equals_ext)
    backspacebtn.grid(column=5,row=4)

    leng = len(equation_text)

    if leng > 15:
        display.config(font="Helvetica 21 bold", pady=12)
        display.update()
        root.update()
...
def basic():
    ...
    size = "314x455"
    root.geometry(size)
    equalsbtn.config(width="226",image=equals_new_basic)
...
pi = Button(frame, image=piimg, width=70, height=69,command=piPress)
...
pow_two = Button(frame, image=pow_of_two_img, width=70, height=69,command=pow_two_func)
...
backspacebtn = Button(frame, image=backspace, width=69, height=67,command=delete)
backspacebtn.grid(column=4,row=4)

equalsbtn = Button(root, image=equals_new_basic, width=226, height=67,command=equals,bg="#262626",fg="#262626")
equalsbtn.place(y=365)
